...
namespace TestingCodeConsoleApp
{
            ...
            while (mapDistanceArray[yEndPos, xEndPos] == emptyValue)
            {
                ...
                for (int y = 0; y < mapHeight; y++)
                    for (int x = 0; x < mapWidth; x++)
                        ...
                        { mapDistanceArray[y, x] = currentDistance; NewPathQueue(x, y); }
            }
    ...
    //ImportBitmap b = new ImportBitmap();
    //string path = @"c:\potato.bmp";
    //b.Import24Bitmap(path);
            

    //        a.GenerateNewMap(16,16,0,0x0000ff,b.BMPMapArray);
    //        a.GeneratePath(2,2,13,11,false);
    //        a.DrawMap();
    #endregion
}
